...
class UNet(nn.Module):
    def __init__(self, input_channel, retrain=True):
        ...
        self.up_conv_2 = ConvTranspose2d(input_channel=1024, output_channel=256)
        # self.conv_de2 = ConvBlock(input_channel=512, output_channel=256)
        self.up_conv_3 = ConvTranspose2d(input_channel=512, output_channel=128)
        # self.conv_de3 = ConvBlock(input_channel=256, output_channel=128)
        self.up_conv_4 = ConvTranspose2d(input_channel=256, output_channel=64)
        self.up_conv_5 = ConvTranspose2d(input_channel=128, output_channel=64)
    ...
    def forward(self, x):
        ...
        up_conv_2 = self.up_conv_2(torch.cat([cropped, up_conv_1], dim=1))
        cropped = self.crop(input_tensor=conv_en3, target_tensor=up_conv_2)
        # conv_de2 = self.conv_de2()
        up_conv_3 = self.up_conv_3(torch.cat([cropped, up_conv_2], dim=1))
        cropped = self.crop(input_tensor=conv_en2, target_tensor=up_conv_3)
        up_conv_4 = self.up_conv_4(torch.cat([cropped, up_conv_3], dim=1))
        cropped = self.crop(input_tensor=conv_en1, target_tensor=up_conv_4)
        up_conv_5 = self.up_conv_5(torch.cat([cropped, up_conv_4], dim=1))
        output = self.conv_1x1(up_conv_5)
        return output
